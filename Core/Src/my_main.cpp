...
volatile uint16_t blueButtonPressed = 1;
double timeConstant = 0;
double timeConstantCalc = 0;
double deviation = 0;
uint16_t numberMeasurements = 1;
double potUsed = 0;
  ...
  if (GPIO_Pin == GPIO_PIN_USER_BUTTON)
  {
    ...
    if (currentTime - lastButtonPressTime > DEBOUNCE_THRESHOLD)
    {
      ...
      else if (currentState == READY)
      {
        ...
        std::string msg2 = "R = " + std::to_string(static_cast<int>(resistance)) + "kOhm";
        event_queue.push(std::make_unique<DisplayEvent>(lcd, msg1 + msg2, msg1.length() - 1));
  ...
  else if (GPIO_Pin == GPIO_PIN_BLUE_BUTTON)
  {
	...
	if (currentTime2 - lastButtonPressTime2 > DEBOUNCE_THRESHOLD)
	{
	  ...
	  if (currentState == ASK_CAPACITOR)
	  {
	    ...
	    event_queue.push(std::make_unique<DisplayEvent>(lcd, "Capacity (uF):  " + msg , 15));
	  }
	  else if (currentState == ASK_RESISTANCE)
	  {
		...
		event_queue.push(std::make_unique<DisplayEvent>(lcd, "Resistance: {" + std::to_string((int)resistance) + "} kOhm", 11));
	  }
